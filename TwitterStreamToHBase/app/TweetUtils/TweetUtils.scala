  ...
  def dumpTweetDatatoHbase(x: Status, consumerKey: String, consumerSecret: String, accessToken: String, accessTokenSecret: String): Unit = {
    ...
    Option(x.getPlace) match {
      case Some(v) => {
        ...
        Option(x.getPlace.getBoundingBoxCoordinates) match {
          case Some(v) =>
            {
              var longisum=0.0
              v.head.foreach(longisum += _.getLongitude)
              val longiavg= longisum/v.head.length
              p.add(TwitterDataColumnFamilies.Place, TwitterDataColumnQualifiers.PlaceLongitude, toBytes(longiavg.toString))

              var latisum=0.0
              v.head.foreach(latisum += _.getLatitude)
              val latiavg= latisum/v.head.length
              p.add(TwitterDataColumnFamilies.Place, TwitterDataColumnQualifiers.PlaceLatitude, toBytes(latiavg.toString))
              //println("RowKey: "+x.getId.toString+" Tweet location Longitude: "+longiavg.toString +" Latitude: "+latiavg.toString)
            }
          case None => "nothing"
        }
...
object TwitterDataColumnQualifiers extends Enumeration {
  ...
  val PlaceLongitude = Bytes.toBytes("placelongitude")
